...
; --- Setup patches ---
to setup-patches
  ; Setup the environment (patches), using colors to define dirty and cleaned cells.
...
; --- Update desires ---
to update-desires
  ; Update agent's desires.
  ; If there is any garbage left, its desire is to clean all the dirt.
  ; If it realises that there is no more dirt, its desire is 'stop and turn off'.
...
to update-beliefs
 ...
 ; This belief set is updated frequently according to the cleaning actions: if the dirt is cleaned, there is no garbage left at that location.
 ask vacuum 0
 [
